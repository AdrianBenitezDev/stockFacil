...
export async function updateEmployeeViewStockPermission(uidEmpleado, puedeVerStock) {
  const uid = String(uidEmpleado || "").trim();
  if (!uid) {
    return { ok: false, error: "Empleado invalido." };
  }
  ...
  try {
    const result = await updateEmpleadoProductoPermissionCallable({ uidEmpleado: uid, puedeVerStock });
    return {
      ok: true,
      data: result?.data || null
    };
